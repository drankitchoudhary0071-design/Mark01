...
End-to-end algorithmic backtest for **PAXG/USDT** (gold proxy on Binance) implementing the Pure Math Trading System (PMTS), an improved variant, and an independent gold mean-reversion pullback strategy.

## Quick start

```bash
pip install -r gold_backtest/requirements.txt
python -m gold_backtest.src.fetch_data          # download 1y 1H + 5m CSVs
...
2. **PMTS** — 1H order blocks → fib golden zone → 5m BOS → candlestick confirmation
...
6. **New strategy** — Gold MR pullback (1H drift + 5m z-score), same evaluation protocol
